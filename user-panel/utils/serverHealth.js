...
/**
 * Single health ping with timeout (fetch — avoids shared axios timeout).
 */
...
/**
 * Wake sleeping host (esp. Render free). Retries until healthy or budget used.
 * @param {object} opts
 * @param {number} [opts.maxMs=75000] total wait budget
 * @param {(info:{attempt:number,elapsed:number,remote:boolean,url:string})=>void} [opts.onProgress]
 */
export async function wakeServer(opts = {}) {
  ...
  while (Date.now() - started < maxMs) {
    ...
    // First pings short; later ones longer while host boots
    ...
    // Gap between retries (gives free-tier dyno time to boot)
...
/** Human status line for login UI */
